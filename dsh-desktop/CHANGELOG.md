...
- **Skill 市场**：Skill 管理新增“已安装 / Skill 市场”双页签，支持 skills.sh 目录、
  搜索、`SKILL.md` 预览和安全安装；内置离线目录并自动回退本地缓存，可配置国内/
  自建目录镜像与 GitHub API 加速地址。安装固定 commit，拒绝符号链接、子模块、
  越界路径和超限文件，校验 Git blob 哈希后原子写入，首次安装默认关闭。
- **模型方案切换**：保留官方“模型管理”同时维护多家供应商/API/模型的能力，新增相邻
  “模型切换”页，可保存最多 100 个不含密钥的命名方案并一键切换当前会话与全局默认；
  任一主 Agent 或子 Agent 正在运行时，客户端锁定按钮且宿主二次拒绝切换。

### 改进
- 桌面产品名统一为 **DeepSeek Harness Desktop**，应用版本升级至 **v3.0.1**；
  Windows 窗口标题、顶部栏、菜单、托盘、快捷方式和安装包名称同步更新，并兼容沿用
  旧版 EAC v2.0 数据目录及旧发布资产命名。
...
- IDE 布局通过 web profile 的受管 patch 替换官方 `ui-layout`，不修改全局官方包；
  启动时幂等同步，可随官方升级回退；文件页和收起窄栏均在左下角固定保留官方设置入口。
...
- 新增路径/同源策略、布局 patch、Skill/MCP 管理、Skill 市场安全安装、插件插槽与跨平台启动回归测试，
